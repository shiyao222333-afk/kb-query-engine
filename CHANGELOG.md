...
### Added
- watch_v2：守望文件夹 v2 初版实现（统一收件箱 + JSONL 状态追踪 + 内容驱动保留策略）
- text_pipeline：新增 `analyze_page_content()` 逐页内容分析函数（5 信号判定可删性）
- config：watch_v2 全量 YAML + 环境变量双源配置（17 个配置项）
- hub 页面：watch_v2 状态监控入口
- watch_v2：队列溢出文件定期救援扫描 `_rescue_orphaned_files()`
