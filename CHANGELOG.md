...
- **VFY-005** `run.bat` 扁平结构重写已通过 L4 用户验收
  - 12 处 `%ERRORLEVEL%` 独立求值，无嵌套块语法错误
  - 正常退出后 `goto :eof` 防止落入 `:error_exit` 误报错误
  - 全流程 Steps 1-8 全部通过，模型预热 37.37s，Web UI 正常启动
...
### Changed
- **代码质量清理** — 死代码删除 + 包拆分
  - 删除 `warmup.py`（PaddleOCR/Ollama 预热脚本，零引用）
  - `watcher_v2.py` (1509行) → `watcher/` 包拆分为 7 个模块
    - `__init__.py` (183行) 公开 API 层
    - `state.py` (198行) 全局状态 + 状态文件操作
    - `utils.py` (138行) 路径定义 + 基础设施检查
    - `failures.py` (232行) 15 种故障 × 5 种策略
    - `processor.py` (549行) 文件处理管线
    - `listener.py` (425行) 文件监听 + 处理循环
    - `migration.py` (134行) 旧版 v1 迁移
  - `pages/hub.py` (1427行) → `pages/hub/` 包拆分为 8 个模块
    - `__init__.py` (84行) 主入口
    - `helpers.py` (96行) 共享工具函数
    - `overview.py` (253行) 概览标签
    - `browse.py` (333行) 浏览标签
    - `review.py` (121行) 待审核标签
    - `inbox.py` (167行) 收件箱标签
    - `dlq.py` (272行) 死信队列标签
    - `detail.py` (140行) 文档详情页
  - 文档文件 `watcher_v2` → `watcher` 引用更新（4 文件）
  - 关键修复：拆分子模块时遗漏 `asyncio`/`STATE`/`log_activity` 导入（4 文件补全）
  - 关键修复：`watcher/__init__.py` 使用 `watcher.state` 模块属性直访避免 import binding 问题
...
- 修复 `run.bat` Step 5 调用 `qdrant_helper.ps1 -Action start` 失败（Unknown action: start）
  - 根因：`qdrant_helper.ps1` 只支持 `detect`/`install`/`health` 三个 action，没有 `start` action
  - 新增 `start` action：从临时文件读取 Qdrant 路径 → 启动 Qdrant → 健康检查（30 次重试/60s）
- 修复 `Found Qdrant: D` 路径显示被截断
  - 根因：`Write-DetectResult` 用 UTF-8 without BOM 写文件，`for /f` 读取时编码不匹配导致路径截断
  - 修复：`run.bat` 改用 PowerShell 读取临时文件（`Get-Content -Encoding UTF8`）
  - 修复：`qdrant_helper.ps1` `start` action 读取临时文件时加 `-Encoding UTF8` 参数
...
- 修复 `run.bat` 启动时多个 cmd.exe 解析错误（Unicode 字符 + PowerShell 内嵌 + 括号嵌套连锁反应）
- 修复 `run.bat` Step 1 端口 8080 清理不可靠（Bug #471）
  - 根因：`Stop-Process -Force` 杀不掉某些进程 + `netstat | findstr` 检测不准确（`:8080 ` 单空格不匹配多空格输出）
  - 新增 `scripts/port_cleanup.ps1`：使用 `Get-NetTCPConnection` 精准检测 + `taskkill /F` 三层杀戮 + 等待端口释放验证
  - `run.bat` Step 1 改为调用 `port_cleanup.ps1`，清理失败则拒绝启动（不再撞车）
- 修复 `run.bat` Step 5 Qdrant 僵尸进程未处理（Bug #471）
  - `qdrant_helper.ps1` `detect` 动作新增健康检查：进程存在但无响应 → 返回 `ZOMBIE`
  - `run.bat` 检测到 `ZOMBIE` 时自动杀掉僵尸进程并重新启动 Qdrant
- 修复 `run.bat` Step 3 配置哈希文件写入权限错误（Bug #471）
  - `local_data\pipe_cfg_stamp.txt` → `%TEMP%\citrinitas_pipe_cfg_stamp.txt`（避免权限问题）
- 修复 `run.bat` `for /f` 读取含空格路径失败 → 统一加 `usebackq` 参数
  - 根因：`chcp 65001` + `enabledelayedexpansion` 下，`( ) !` 和 Unicode 字符被 cmd.exe 误解析
  - 移除启动时 LLM API 检查（应用启动不依赖 LLM，运行时遇缺失会优雅降级）
  - Unicode 字符（em dash `—`、箭头 `→`）替换为纯 ASCII，消除多字节 UTF-8 误解析
  - `[!]` 在括号块内改为 `[^^!]`（`!` 被 delayed expansion 吞掉）
- 修复 `run.bat` Step 1 未清理 Qdrant 残留进程导致健康检测误判（Bug #471 收尾）
  - 根因：上一轮 `run.bat` 崩溃退出后 `qdrant.exe` 未正确终止，残留进程被误判为"健康"
  - Step 1 新增 `taskkill /F /IM qdrant.exe`，与端口 8080 清理同级处理
  - `qdrant_helper.ps1` detect 动作健康检查改用 `/collections`（比 `/health` 通用，所有 Qdrant 版本均支持）
  - `main.py` startup() 新增 Qdrant 连接重试（3 次，每次等 5 秒），避免启动顺序竞争导致的短暂不可达
- 修复 `main.py` 启动时 Qdrant 不可用直接退出的问题（Bug #471 收尾）
  - 根因：模型预热耗时 ~38 秒，期间 Qdrant 若未完全就绪会导致连接失败
  - 改为重试 3 次，仍失败则明确报错退出（不再静默继续）

  - 删除 `scripts/check_llm.ps1`（不再需要）
- 修复 Qdrant 健康检查不可靠问题（重构为三场景覆盖）
  - `qdrant_helper.ps1` 新增 `health` action，使用 `.NET TcpClient`（与 detect action 相同可靠方案）
  - 场景一 "已在运行"：快速检查（3 次重试，6s），失败仅警告不阻塞
  - 场景二 "刚启动"：完整轮询（30 次重试，60s），失败则阻塞退出
  - 场景三 "用户跳过"：无检查
  - 删除旧的 `:check_qdrant_health` 块（依赖 curl.exe，PATH 不可靠）
  - 恢复误删的 `:skip_qdrant` 标签
- 修复 Qdrant detect 假阳性：端口通但无 HTTP 响应时错误标记为 "API_ALREADY_RUNNING"
  - `qdrant_helper.ps1` detect: 改为 `curl.exe /healthz` 返回 `"ok"` 才确认 alive（不再乐观回退）
- 修复 `watcher.py` 关闭时 `WATCH_V2_PROCESS_TIMEOUT` 未定义（NameError）
  - `config/settings.py`: 新增 `WATCH_V2_PROCESS_TIMEOUT` 配置项（默认 30s）
  - `watcher.py`: import 列表中补入 `WATCH_V2_PROCESS_TIMEOUT`
- 修复 Qdrant detect 假阳性（根因修正）：用 `Get-Process qdrant` 替代 TcpClient + curl.exe 双重检查
  - 之前的 "API_ALREADY_RUNNING" 误报来自 TcpClient 连到了端口 6333 上的非 Qdrant 进程，以及 curl.exe 的不可靠响应
  - `Get-Process qdrant` 是唯一不撒谎的检测方式：进程存在 = Qdrant 在运行，不存在 = 没运行
  - 健康检查同样先验证 `Get-Process qdrant`，进程不存在时立即失败（不浪费 60s 轮询）
- 修复 `run.bat` Step 1 端口 8080 清理（根因修正）：`taskkill //PID //F` 的 `//` 前缀对外部 exe 无效
  - `//` 仅对 CMD 内部命令有效，`taskkill.exe` 需要 `/PID` `/F`（单斜杠）
  - 改为单一 PowerShell 调用（`netstat` + `Stop-Process`），彻底消除 batch 嵌套解析问题
  - 之前 5 轮循环每次 `taskkill //PID` 静默失败 + `2>NUL` 抑制错误 = 端口从未被释放
- 修复 `run.bat` Step 5 调用 `qdrant_helper.ps1 -Action start` 失败（Unknown action: start）
  - 根因：`qdrant_helper.ps1` 只支持 `detect`/`install`/`health` 三个 action，没有 `start` action
  - 新增 `start` action：从临时文件读取 Qdrant 路径 → 启动 Qdrant → 健康检查（30 次重试/60s）
- 修复 `Found Qdrant: D` 路径显示被截断
  - 根因：`Write-DetectResult` 用 UTF-8 with BOM 写文件，`for /f` 读取时 BOM 导致路径显示异常
  - 改为用 `[System.IO.File]::WriteAllText()` 写入无 BOM 的 UTF-8 文件
- 修复 `run.bat` 嵌套 `( )` 块中 `%ERRORLEVEL%` 解析时 stale 值导致 `: was unexpected` 语法错误（VFY-005）
  - 根因：`cmd.exe` 解析多行 `( )` 块时，`%ERRORLEVEL%` 在解析阶段展开而非执行阶段
  - 修复：重写为完全扁平结构（`goto` 跳转代替嵌套块），12 处 ERRORLEVEL 检查均独立求值
- 修复 `run.bat` 正常退出后错误落入 `:error_exit` 标签
  - 根因：`cmd /k` 结束后无跳转，顺序执行落入 error handler
  - 修复：在 `:error_exit` 前加 `goto :eof`

---

## [v1.0.1] — 2026-06-28

> **代码质量清理** — 死代码删除 + 大文件拆分 + run.bat 修复。

### Removed
- 删除死代码 `warmup.py`（110 行，无引用）
- 删除死代码 `sync_ima.py`（65 行，无引用）
- 删除死代码 `watcher.py`（旧版，已由 `watcher/` 包替代）

### Changed
- **大文件拆分**: `text_pipeline.py`（1,032 行）→ `text_pipeline/` 包（6 文件）
  - `ocr.py`（469 行）— OCR 引擎
  - `extract.py`（291 行）— 文本提取 + 编码检测
  - `chunk.py`（147 行）— 切块 + 哈希
  - `embed.py`（55 行）— 嵌入向量
  - `analyze.py`（101 行）— 逐页分析
  - `__init__.py`（40 行）— facade 重新导出
  - 所有外部 importer 零感知，一行不改
- **大文件拆分**: `watcher_v2.py`（1,834 行）→ `watcher/` 包（7 文件）
- **大文件拆分**: `pages/hub.py`（1,418 行）→ `pages/hub/` 包（8 文件）

### Fixed
- 修复 `run.bat` 调用已删除的 `warmup.py` 导致启动失败（P0）
- 修复 `run.bat` 步骤编号不一致（[x/8] 实为 10 步 → 统一为 8 步）
- 更新 `run.bat` 版本号 v1.0.0 → v1.0.1
- 更新 `run.bat` 守望目录显示：`data\watch\` → `data\inbox\`
...
- **A4** 守望文件夹 v2：`watcher.py` 统一收件箱 + JSONL 状态追踪 + 内容驱动保留策略 + 15 种故障 × 5 种策略
- **A5** OCR 接入管道：
...
- main：`os._exit(0)` 前调用 `stop_watcher()` 优雅释放锁资源 (#329, B10)
- watch_v2：基础设施故障改为立即 retry 不阻塞处理循环 (#330, B13)
- hub.py：删除无锁 file_state.jsonl 重复实现，统一委托 watcher (#331, B19/B29)
...
- **代码质量重构**：
  ...
  - `watcher.py`：将 `import kb_query` 从函数体内移到模块顶部
  - `kb_query.py`：新增 `route_by_confidence()` 共用函数（置信度三档路由）
  - `pages/ingest.py` / `watcher.py`：使用 `kb_query.route_by_confidence()` 替换内联逻辑，消除重复
  - `ingest_pipeline.py`：提取 `_prepare_metadata()`、`_build_point()`，修复未使用导入
  - `watcher.py`：提取 `_do_prechecks()`、`_do_ocr_fallback()`、`_do_classify()`、`_do_ingest()`、`_do_post_ingest()`，修复语法错误
...
- **A4 守望文件夹 P0-P2 bug 修复**（v1.0.0 最终验证前审查发现）：
  ...
  - **优雅关闭支持**（F6 — P2）：
    - 新增 `_signal_handler()` 信号处理器，`start_watcher()` 注册 `SIGINT`/`SIGTERM`
    - `stop_watcher()` 等待处理线程退出（最多 `PROCESS_TIMEOUT` 秒）
...
### Fixed
- 🐛 8 个严重 Bug + 7 个重要 Bug（已全部修复，详见 v1.0.0 Bug 修复记录）
