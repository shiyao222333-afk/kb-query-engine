...
- **S1 混合查询**：原生 Qdrant Query API（prefetch + RRF fusion），稠密向量 + BM25 稀疏向量并行搜索
- **S1.3 Grouping API**：按 `doc_id` 分组去重，每文档只保留最佳 chunk
- **S1.4 量化**：int8 标量量化，内存降低约 75%
- **S1.5 ACORN 过滤**：搜索带过滤条件时自动启用（`enable=true, max_selectivity=0.4`），提升严格过滤下的召回率
- **S2 重排序**：嵌入模型对 Top-K 结果重新打分排序
- **S3 重排序可配置**：引擎配置页面新增「🔀 重排序」标签页（开关/模型/Top-N）
...
- 搜索优化：重排序模型可选配置（引擎配置页面设置）
- 审核队列：置信度路由落地（≥0.75 直接入库 / 0.40–0.75 待审核 / <0.40 死信队列）
- 审核队列：知识中枢页面新增「待审核」标签页
- 审核队列：死信队列查看/编辑/重新入库功能

### Fixed
- OCR 功能完全不可用（`PaddleOCR` 未安装在项目 venv 中）— 安装 PaddleOCR 3.7.0 + PPStructureV3 + paddlepaddle-gpu 3.3.0（CUDA 12.6，启用 RTX 3080 GPU 加速）
- PPStructureV3 依赖缺失 — 安装 `paddlex[ocr]` 额外依赖，结构化识别正常（文字+表格+公式+图片区域）
- GPU 模式不可用 — 卸载 CPU 版 paddlepaddle 3.0.0，安装 GPU 版 paddlepaddle-gpu 3.3.0
- 点击「开始识别」按钮提示"请先选择图片"— 根因：`auto_upload=False` 时 `on_upload` 事件不触发，`ocr_file_data` 始终为 None

### Changed
- 删除独立「📷 OCR 截图」Tab，合并到「📎 文件上传」Tab — 图片文件上传后显示「开始识别」按钮，手动点击才执行 OCR（统一入口，用户控制感更强）
- 版本路线调整：v0.8.0 = 搜索优化 + 审核队列；知识关系网推迟至 v1.1.0
