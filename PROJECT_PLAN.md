...
最后更新: 2026-06-25 (新增 v1.10.0 Git 页面 + 路线图终版确认)

---

## 当前状态

- 当前版本：**v1.0.0 ✅ 已完成（L4 用户验收通过，2026-06-28）**
- 活跃 Bug：**0**（VFY-001~005 全部修复并验证）
- 下个版本：**v1.1.0** — 闪念笔记（待启动）
- Git 状态：main 分支，本地有未提交修改（CHANGELOG + PROJECT_PLAN）
...
| v1.0.1 | 🔮 | 交付 | 代码质量清理 | 死代码删除（3文件）+ watcher_v2.py/hub.py 拆分 |
| v1.1.0 | 🔮 | 交付 | 闪念笔记 | fleeting_note 类型 + 快速捕获 + 轻量分类 |
| v1.2.0 | 🔮 | 交付 | 错误日志规范 | 统一日志格式 + 错误码体系 + 日志查看器 |
| v1.3.0 | 🔮 | 交付 | 项目间通信 | REST API 正式启用 + 与 OpusMagnum 对接 |
| v1.4.0 | 🔮 | 交付 | 知识关系网 + 检索增强 | NetworkX 知识图谱可视化 + QA 自动生成 + LLM 关系发现 |
| v1.5.0 | 🔮 | 交付 | 入库翻译 | LLM 翻译管道 — 外语统一翻译后入库 |
| v1.6.0 | 🔮 | 交付 | LLM 智能选择 | 用户可选云LLM / 本地小模型 / 机械程序，附建议 |
| v1.7.0 | 🔮 | 交付 | 性能优化 | 摄入管线加速 + 嵌入缓存 + 启动提速 |
...
### v1.2.0 🔮 错误日志规范（2026-06-25 研究 — 待规划）
...
| **watcher 日志黑盒** | watcher_v2.py 1834 行，异常处理不完整 | 守望出问题用户毫不知情 |
...
1. **嵌入批处理**：Ollama API 支持批量 embedding，一次调 10 块
2. **LLM 调用缓存**：相同内容 hash 命中则跳过分类（`content_hash` 已有）
3. **启动并行化**：Qdrant + Ollama 同时启动，健康检查并行
4. **OCR 懒加载**：PaddleOCR 按需加载（非 OCR 文件不加载模型）
5. **管线步骤并行**：非依赖步骤可以互不等待（chunk + LLM class 可同时进行）
6. **嵌入缓存**：已计算过的嵌入向量用 hash 做 key 缓存（热缓存命中 >90%）
...
当前 Citrinitas `/health` 返回了 `qdrant_online`、`watcher_v2` 等内部状态，
...
与 v1.1.0 闪念笔记有协同效应：用户快速输入灵感后，如果搜索也能智能理解意图，体验连贯。

**建议**：在 v1.1.0（闪念笔记）或 v1.4.0（知识关系网）时同步评估此功能。
...
| v1.0.1 | 死代码清理 + 大型文件拆分 | watcher.py/ sync_ima.py/ warmup.py 删除 + watcher_v2.py/hub.py 拆分 |
...
| 8 | `watcher.py` 死代码（被 `watcher_v2.py` 完全替代） | `watcher.py` (894行) | 🟡 P1 | ✅ 已清理 (v1.0.1) |
| 9 | `sync_ima.py` 死代码（IMA 同步脚本，从未激活） | `sync_ima.py` (454行) | 🟢 P2 | ✅ 已清理 (v1.0.1) |
| 10 | `warmup.py` 死代码（预热脚本，未被引用） | `warmup.py` (110行) | 🟢 P2 | ✅ 已清理 (v1.0.1) |
| 11 | `watcher_v2.py` 单文件 1834 行，职责过多 | `watcher_v2.py` | 🔴 P0 | 📋 待拆分 (v1.0.1) |
...
| 文件 | 行数 | 原因 |
|------|------|------|
| `watcher.py` | 894 | 已被 `watcher_v2.py` 完全替代，无任何引用 |
| `sync_ima.py` | 454 | IMA 知识库同步脚本，从未被激活引用 |
| `warmup.py` | 110 | PaddleOCR/Ollama 预热脚本，无任何引用 |

**B. `watcher_v2.py` 拆分**（1834行 → `watcher_v2/` 包）

```
watcher_v2.py (1834行) → watcher_v2/ 包
...
- `wc -l watcher_v2/*.py pages/hub/*.py` 单文件 ≤ 400行
