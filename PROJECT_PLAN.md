...
> 当前 `kb_query.py` 3958 行、`main.py` 1213 行，职责混乱、超长函数多。
...
| # | 问题 | 位置 | 严重程度 |
|---|---|---|---|
| 1 | `kb_query.py` 五层职责混在一起（Qdrant 操作/文本处理/分类管道/检索问答/文档管理） | `kb_query.py` | 🔴 P0 |
| 2 | `page_ingest()` 366 行，UI+逻辑不分 | `main.py:294` | 🔴 P0 |
| 3 | `ingest()` 302 行，做了分块+嵌入+存储三件事 | `kb_query.py:1412` | 🟡 P1 |
| 4 | `classify_document()` 159 行，三层管道挤在一起 | `kb_query.py:2361` | 🟡 P1 |
| 5 | 返回值格式不统一（dict / 抛异常 / None） | 全局 | 🟡 P1 |
| 6 | `panel_funcs.py` 编辑对话框 99 行，可拆分 | `panel_funcs.py:154` | 🟢 P2 |
